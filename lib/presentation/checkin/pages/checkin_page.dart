        ...
        builder: (context, state) {
          ...
          return SingleChildScrollView(padding: const EdgeInsets.all(20), child: Column(
            crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text('GPS sẽ được lấy tự động từ thiết bị khi bấm nút trên',
                style: TextStyle(fontSize: 11.5, color: AppColors.getTextSecondary(isDark))),
              const SizedBox(height: 24),
              Text('Lịch sử check-in/out', style: TextStyle(fontSize:14,fontWeight:FontWeight.w700,color:AppColors.getTextPrimary(isDark))),
              const SizedBox(height: 10),
              if (history.isEmpty)
                Container(padding: const EdgeInsets.all(20), alignment: Alignment.center,
                  decoration: BoxDecoration(color: AppColors.getSurfaceGrey(isDark), borderRadius: BorderRadius.circular(14)),
                  child: Text('Chưa có lịch sử', style: TextStyle(color: AppColors.getTextSecondary(isDark))))
              else
                ...history.map((h) => Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(h.typeLabel, style: TextStyle(fontWeight: FontWeight.w600, fontSize: 13, color: AppColors.getTextPrimary(isDark))),
                      Text(h.gpsString, style: TextStyle(fontSize: 11, color: isDark ? AppColors.textHintDark : AppColors.textHint)),
                    ])),
                    Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      Text(_fmt(h.timestamp), style: TextStyle(fontSize: 11, color: AppColors.getTextSecondary(isDark))),
