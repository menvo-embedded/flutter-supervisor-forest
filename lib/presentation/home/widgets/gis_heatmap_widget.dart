...
const List<LatLng> _samplePlots = [];
...
class _LayerConfig {
  ...
  final Color color;
  bool visible;
  _LayerConfig(this.type, this.label, this.emoji, this.color,
      {this.visible = true});
}
...
class _GISHeatmapWidgetState extends State<GISHeatmapWidget> {
  ...
  final _layers = [
    ...
    _LayerConfig(
        _LayerType.activity, 'Hoạt động', '⚡', const Color(0xFFF59E0B)),
    _LayerConfig(
        _LayerType.satellite, 'Vệ tinh', '🛰️', const Color(0xFF8B5CF6),
        visible: false),
    _LayerConfig(
        _LayerType.basemap, 'Bản đồ nền', '🗺️', const Color(0xFF64748B)),
  ];
  ...
  Future<void> _fetchSupabaseData() async {
    ...
    try {
      ...
      try {
        ...
        final ownersRes =
            await _supabase.from('forest_owners').select('id, owner_code');
      ...
      _supabaseProjects = rawProjects
        ...
        if (statusVal.toLowerCase() == 'approved' ||
  ...
  Widget build(BuildContext context) {
    ...
    if (_errorMessage != null) {
      return SizedBox(
        height: 380,
        child: Center(
          child: Text("Lỗi tải bản đồ: $_errorMessage",
    ...
    return Column(children: [
                      ...
                      children: [
                        Row(children: [
                          ...
                          Container(
                            ...
                            child: Text('${_supabaseProjects.length}',
      ...
      Row(children: [
        _StatChip(
            ...
            value:
                '${_fmtNum(_supabaseProjects.fold(0.0, (s, p) => s + p.areaHa))} ha',
            color: AppColors.primary),
        const SizedBox(width: 8),
        _StatChip(
            label: 'Carbon',
            value:
                '${_fmtNum(_supabaseProjects.fold(0.0, (s, p) => s + p.carbonStock))} tCO₂e',
            color: AppColors.primaryMid),
        const SizedBox(width: 8),
        _StatChip(
            label: 'Dự án',
            value: '${_supabaseProjects.length}',
            color: AppColors.primary),
      ]),
    ]);
  }
...
class _FullscreenMapPage extends StatefulWidget {
  ...
  final void Function(bool isSat, ForestProject? sel, List<_LayerConfig> layers)
      onStateChanged;
            ...
            child: Row(children: [
              ...
              GestureDetector(
                onTap: () => setState(() {
                  ...
                  final satLayer =
                      _layers.firstWhere((l) => l.type == _LayerType.satellite);
              ...
              child: Container(
                ...
                child: Row(children: [
                  _FsStat(
                      label: 'Diện tích',
                      value:
                          '${_fmtNum(widget.projects.fold(0.0, (s, p) => s + p.areaHa))} ha',
                      color: AppColors.primary),
                  _vDivider(),
                  _FsStat(
                      label: 'Carbon',
                      value:
                          '${_fmtNum(widget.projects.fold(0.0, (s, p) => s + p.carbonStock))} tCO₂e',
                      color: AppColors.primaryMid),
                  _vDivider(),
                  _FsStat(
                      label: 'Dự án',
                      value: '${widget.projects.length}',
                      color: AppColors.primary),
                ]),
              ),
            ),
...
class _MapView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final showProjects =
        layers.any((l) => l.type == _LayerType.project && l.visible);
    final showSamplePlt =
        layers.any((l) => l.type == _LayerType.samplePlot && l.visible);
    ...
    return FlutterMap(
      ...
      children: [
        TileLayer(
            urlTemplate: tileUrl,
            userAgentPackageName: 'com.qlr.forest',
            maxZoom: 18),
        if (showProjects)
          CircleLayer(
              circles: projects
...
class _LayerPanel extends StatelessWidget {
  ...
  const _LayerPanel(
      {required this.layers, required this.isDark, required this.onToggle});
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 6),
              ...layers.take(3).map((l) => _LayerChip(
                  config: l, isDark: isDark, onToggle: (v) => onToggle(l, v))),
              Padding(
                padding: const EdgeInsets.symmetric(vertical: 4),
                child: Divider(height: 1, thickness: 1, color: dividerColor),
              ),
              ...layers.skip(3).map((l) => _LayerChip(
                  config: l, isDark: isDark, onToggle: (v) => onToggle(l, v))),
            ],
          ),
        ),
...
class _LayerChip extends StatelessWidget {
  ...
  const _LayerChip(
      {required this.config, required this.isDark, required this.onToggle});
  ...
  Widget build(BuildContext context) {
    return AnimatedScale(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        CustomPaint(
            painter: _PinTailPainter(color: color), size: const Size(10, 8)),
      ]),
    );
  }
}
    ...
    return Container(
      ...
      child: Column(
          ...
          children: [
            ...
            _infoRow(
                'Carbon', '${project.carbonStock.toStringAsFixed(0)} tCO₂e'),
            _infoRow(
                'Trạng thái',
    ...
    return Expanded(
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(label,
              style: TextStyle(
                  fontSize: 10, color: AppColors.getTextSecondary(isDark))),
        ]),
      ),
    );
  }
