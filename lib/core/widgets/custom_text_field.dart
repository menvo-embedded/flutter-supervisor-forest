...
class _State extends State<CustomTextField> {
  ...
  Widget build(BuildContext context) {
    ...
    final surfaceColor = widget.readOnly ? AppColors.getSurfaceGrey(isDark) : AppColors.getSurface(isDark);
    final borderColor = AppColors.getBorder(isDark);

    return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
      Text(widget.label,style:TextStyle(fontSize:13,fontWeight:FontWeight.w500,color:textPrimary)),
      ...
      TextFormField(
        ...
        decoration:InputDecoration(
          hintText:widget.hint,hintStyle:TextStyle(fontSize:14,color:textHint),
          prefixIcon:widget.prefix,
          suffixIcon:widget.obscureText
            ?IconButton(icon:Icon(_hide?Icons.visibility_off_outlined:Icons.visibility_outlined,size:20,color:textSecondary),
